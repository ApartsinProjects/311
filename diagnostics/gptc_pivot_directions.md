...
Perspectivist NLP already argues against forcing all annotator disagreement into a single truth. Röttger et al. explicitly distinguish descriptive annotation, which captures beliefs, from prescriptive annotation, which implements a particular policy. :contentReference[oaicite:0]{index=0}
...
Recent work has pushed construct validity into ML benchmark design, arguing that benchmark scores only support scientific claims insofar as the operationalized measurement actually corresponds to the target construct. :contentReference[oaicite:1]{index=1}
...
There is substantial work on LLM-based evaluators. G-Eval showed strong correspondence with human judgments but also evaluator biases. :contentReference[oaicite:2]{index=2} JudgeBench specifically argues that LLM judges themselves need rigorous evaluation and tests judges on objectively determined response comparisons. :contentReference[oaicite:3]{index=3} Panels of heterogeneous judges have also been proposed because individual judges exhibit biases and variance. :contentReference[oaicite:4]{index=4}
...
Superset/partial-label learning has existed for years; Liu & Dietterich formalized learnability of superset-label learning at ICML 2014. :contentReference[oaicite:5]{index=5}
...
ChaosNLI already showed that NLI examples with high human disagreement account for a disproportionate share of model errors, so “humans disagree where models fail” by itself is not novel. ChaosNLI collected 100 annotations per item and showed strong model degradation on low-agreement items. :contentReference[oaicite:6]{index=6}
...
Conformal prediction is a natural tool, but conformal prediction under ambiguous ground truth is already established, including a TMLR 2023 framework explicitly addressing ambiguous labels. :contentReference[oaicite:7]{index=7}
...
MIMIC-IV-ED contains a free-text `chiefcomplaint` recorded at triage and subsequent ICD diagnoses assigned by trained coders after the ED visit for billing. :contentReference[oaicite:8]{index=8}
...
BANKING77 contains fine-grained banking customer-service intents; CLINC150 has 150 in-scope intents plus out-of-scope queries; HWU64 provides another fine-grained intent taxonomy. CLINC150, for example, explicitly contains 150 classes with 100 train, 20 validation, and 30 test examples per in-scope intent. :contentReference[oaicite:9]{index=9}
...
Civil Comments exposes fractional toxicity labels corresponding to annotator vote fractions. :contentReference[oaicite:10]{index=10}

The Jigsaw Specialized Rater Pools release is even more useful: it contains 25,500 comments and 382,500 disaggregated annotations across rater groups. :contentReference[oaicite:11]{index=11}
...
ChaosNLI has approximately 4,645 items drawn from SNLI/MNLI/αNLI with **100 human annotations per item**. :contentReference[oaicite:12]{index=12}
...
MASSIVE contains more than one million utterances, 60 intents, 55 slot types, and 52 languages. :contentReference[oaicite:13]{index=13}
...
EURLEX57K contains about 57k EU legal documents tagged with roughly 4.3k EUROVOC concepts and is inherently multi-label. :contentReference[oaicite:14]{index=14}
...
Perspectivist NLP explicitly argues that disagreement can be meaningful and surveys methods preserving disaggregated labels rather than collapsing them into one gold standard. :contentReference[oaicite:15]{index=15}
...
Again, ambiguous-ground-truth conformal prediction already exists. :contentReference[oaicite:16]{index=16} Your novelty would come from the dual semantic/administrative formulation and certified label sets.
